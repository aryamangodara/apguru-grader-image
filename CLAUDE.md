...
**Google Gemini** is the LLM, called either through **Vertex AI** (default) or the AI Studio API key. **MySQL** is the datastore. **Langfuse** is **mandatory** LLM tracing — every Gemini call must be traced, so the app fails fast at startup without `LANGFUSE_*` credentials configured and refuses to make an LLM call when tracing isn't active (**no Langfuse, no LLM call**; see `app/core/observability.py::configure_langfuse` / `require_langfuse_active`). A failed Langfuse *auth check* at startup only warns (a Langfuse outage won't block boot); missing keys still hard-fail.
...
- **Typed:** answers supplied inline as JSON → answer keys reconciled with the rubric (`remap_typed_answers`, below) → LLM-labelled by subpart against the rubric structure (`label_typed_answers`) → graded.

**Typed answer-key reconciliation (`app/services/grader_answer_map_service.py`).** Typed grading matches answers to the rubric by an exact key lookup, so a client that keys answers by its own numbering (e.g. `"21".."24"` for a rubric keyed `"216a".."220"` — see prod quiz 861) used to score a silent, misleading `0/N succeeded`. Before labelling, `remap_typed_answers` gates on the keys: if every submitted key is already a rubric question id it trusts them (no LLM); otherwise ONE Gemini call (`map_answers_to_questions` in `app/services/grader/answer_map.py` + `prompts/answer_map.txt`, model `grader_answer_map_model`, traced as `grader.answer_map`) matches each answer to the rubric question(s) it answers **by content** (`prompt_summary` + criteria; one blob may cover several sub-parts). It **fails loud** — `AnswerMappingError` → job `failed` with `error_message` `ANSWER_MAPPING_FAILED: …` — if any answer maps below `grader_answer_map_min_confidence`, two answers collide on one question, or an answer matches nothing, rather than emit a plausible-but-wrong score. On a clean remap the answers are rekeyed onto rubric ids, the touched questions are flagged for review, and the applied mapping is recorded on the scorecard as `answer_mapping`. Toggle with `grader_enable_answer_map` (default on). Typed rubric path only (handwritten already maps via OCR; the rubric-free knowledge path has no rubric to map against). Like `grade_submission_knowledge`, `map_answers_to_questions` is an APGuru-specific addition to the vendored package (no upstream in `notebooks/Grader`).

**FRQ-only — MCQs are never graded (`grader_exam_service.drop_mcq_questions`).** The grader grades free-response only. In rubric mode, `_do_grade` strips any multiple-choice questions from the cached rubric at grade time — **before** the answer auto-map and `grade_submission` — so MCQs are never scored, shown on the scorecard, or counted in the denominator (fixing e.g. quiz 861 scoring 8/36 instead of 8/16). Detection is conservative: a question is MCQ only if it has rubric points and **all** of them are a bare `Correct Answer: (X)` answer-key criterion (the shape the parser emits for MCQs), so a genuine FRQ is never dropped; the filter works on a copy, leaving the stored `rubric_json` untouched. A rubric left with zero FRQs fails the job (`INVALID_SUBMISSION`, "no free-response questions to grade"). In knowledge mode (rubric-free homework), `prompts/knowledge_grade.txt` instructs the model to skip MCQs and omit them from the scorecard. Applies to every assessment type (exam/homework/quiz).
...
**Assessment registry & rubric cache (`app/services/grader_exam_service.py`).** `register_exam` parses the marking-scheme PDF once (`parse_rubric_pdf`) and stores the `ParsedRubric` JSON in `assessment_registry.rubric_json`; subsequent students reuse it. Before parsing it rejects a `test_id` that isn't a live row in the main app's `tests` table (`assert_source_is_valid` → `INVALID_TEST_ID`, issue #11).

**Assessments = exams + homework + quizzes (one engine, one discriminator).** The same services grade **homework** and **quizzes**: every `assessment_registry` / `grading_job` row carries an `assessment_type` ∈ {`exam`, `homework`, `quiz`} (default `exam`, threaded as a trailing arg so the exam path is unchanged), `assessment_registry` is keyed on the composite `(assessment_type, test_id)` so ids from different source tables can't collide, and `assert_source_is_valid` dispatches the id check to the right main-app table (`exam`→`tests`, `homework`→`docs_homework_test`, `quiz`→`quiz`). The homework/quiz HTTP surface is a **separate, self-contained router** (`app/api/v1/assessment_router.py` → `app/controllers/assessment_controller.py` → the shared services) under `/api/v1/grader/assessments`, keyed on `(assessment_type, source_id)` — `assessment_type` is a **body field** (register/submit) or **query param** (listings), never in the URL path; `source_id` is the main-app row id. It reuses the jobs table, scorecard, and `{error_code, detail}` envelope. The exam endpoints are untouched. The `assessment_type` column (migration 042) and the `ap_exam`→`assessment_registry` table rename (migration 043) are owned by the central migration pipeline.

**Homework without a marking scheme (rubric-free "knowledge" grading).** **Homework** — and only homework — may be registered without `marking_scheme_pdf_url` (exams/quizzes still require one; enforced by `MarkingSchemeRequiredError` → 400 `MARKING_SCHEME_REQUIRED`, and by the `RegisterAssessmentRequest` validator for quizzes). When it's absent, `register_exam` skips the rubric parse (no Gemini call, no Langfuse guard) and stores an **empty `ParsedRubric`** so the `NOT NULL` `rubric_json` column, `get_cached_rubric`, and the `create_job` 409 gate all keep working unchanged — **no DB migration**. The mode is detected by `grader_exam_service.is_rubric_free(exam_row)` (`assessment_type == "homework"` **and** no `marking_scheme_pdf_url`), NOT by inspecting the rubric. At grade time `_do_grade` routes such jobs to `app/services/grader_knowledge_service.py`, which renders the questions PDF (required in this mode — for typed too — so the model sees each question), gets the answers (OCR for handwritten, inline `answers_json` for typed), and makes **one** Gemini call (`grade_submission_knowledge` in `app/services/grader/knowledge.py` + `prompts/knowledge_grade.txt`, model `grader_knowledge_model`) that judges each answer **correct/incorrect/partial** from the model's **own subject knowledge**. It composes the SAME `GradedScorecardResponse` with `grading_mode="knowledge"`, per-question `verdict` + `correct_answer` (explanation in `comment`), an `correct_count`/`questions_total` "X of Y correct" tally, and the marks fields (`percentage`, `points_earned`, `question_wise_marks`) zeroed — **no marks**. The rubric grade path (`grade_submission` / `build_scorecard_response` / audience summaries) is bypassed for these jobs. `grade_submission_knowledge` + its schemas are an APGuru-specific addition to the vendored package (no upstream in `notebooks/Grader`).
...
- **Grader tables:** `assessment_registry` (registered assessments — exams/homework/quizzes — + cached rubric JSON; formerly `ap_exam`) and `grading_job` (per-submission job: status ∈ {queued, running, succeeded, failed}, plus the `scorecard_json`); per-course `grading_addendum` / `ocr_addendum` columns live in `course_configs`.
